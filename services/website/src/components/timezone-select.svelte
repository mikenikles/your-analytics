<script lang="ts">
   ...
   * 2. Scroll to the main table and sort it by "UTC offset" in ascending order
   * 3. Run the following script in the dev tools console.
   *    ```
        copy(Array.from($("#mw-content-text table.wikitable.sortable.jquery-tablesorter:first-of-type tbody tr")).reduce((result, tr) => {
          const tzName = Array.from(tr.children)[2].innerHTML.match(/>(.*.)</)[1];
          const tzOffset = Array.from(tr.children)[5].innerHTML.match(/>(.*.)</)[1];
          result[tzName] = tzOffset;
          return result;
        }, {}))
   *    ```
   */
